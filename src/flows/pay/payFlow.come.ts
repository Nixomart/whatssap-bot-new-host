...
export default addKeyword<Provider, Database>(utils.setEvent("PAY_FLOW_COME"))
  .addAnswer("...⏱")
  .addAction(async (ctx, { flowDynamic, endFlow, state, gotoFlow }) => {
    ...
    if (turnsFixeds.length > 0) {
      ...
      const turnsFixeds2 = turnsFixeds.map((turn) => {
        ...
        const inicioSemanaHoy = hoy.startOf("week").day(0);
      ...
      if (reestablecido == true) {
        console.log("entra a actualizar doc");
      ...
      if (turnosWeek0.length > 0 || turnosWeek1.length > 0) {
        ...
        const turnoMasCercanoWeek0 = turnosWeek0.reduce((minTurno, turno) => {
        ...
        const turnoMasCercanoWeek1 = turnosWeek1.reduce((minTurno, turno) => {
          return turno.daysOfWeek[0] < minTurno.daysOfWeek[0]
            ? turno
            : minTurno;
        }, turnosWeek1[0]);
        ...
        turnoMasCercano =
          turnosWeek0.length > 0 ? turnoMasCercanoWeek0 : turnoMasCercanoWeek1;
    ...
    if (turno == undefined && turnoMasCercano == null) {
      ...
      if (turno === undefined) {
        ...
        if (turnoMasCercano == null) {
          console.log("ENTRA ACA A SETEAR TURNO = TURNO");
          turno = turno;
        } else {
          if (
