...
export default addKeyword<Provider, Database>(utils.setEvent("LIST_PAYMENT_METHODS")).addAction(
  async (ctx, { flowDynamic, endFlow, state, gotoFlow }) => {
    ...
    const medicData = state.getMyState().medic
      ...
      await flowDynamic(
        `💢💢🟢 Tienes turno con: *${medicData.name}* \n📅 El día: *${dayjs(turno.start).format("dddd D, MMMM HH:mm a")}* \n📍 Lugar: *${turno.name}* \n🏣 Dirección: *${turno.address}* \n\n*Elige un método de pago:* \n${paymentMethods.map((pay, index) => `*${index}*. ${pay === "os" ? "Obra Social" : pay === "transfer" ? "Transferencia" : pay === "presential" ? "Presencial" : ""}\n`)}`
      )
